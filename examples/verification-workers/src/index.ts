import { Directory, VerificationParams, helpers, jwkToKeyID, verify } from 'web-bot-auth';
...
const getDirectory = async (): Promise<Directory> => {
	const key = {
		kid: await jwkToKeyID(jwk, helpers.WEBCRYPTO_SHA256, helpers.BASE64URL_DECODE),
		kty: jwk.kty,
		crv: jwk.crv,
		x: jwk.x,
...
async function verifyEd25519(data: string, signature: Uint8Array, _params: VerificationParams) {
...
export default {
	async fetch(request, env, ctx): Promise<Response> {
		...
		if (url.pathname.startsWith('/.well-known/http-message-signatures-directory')) {
			return new Response(JSON.stringify(await getDirectory()), { headers: { 'content-type': 'application/http-message-signatures-directory' } })
		}
