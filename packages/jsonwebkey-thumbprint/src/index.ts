...
export const jwkThumbprintPreCompute = (jwk: JsonWebKey): Uint8Array => {
    ...
    switch (jwk.kty) {
        ...
        case 'EC':
            return encoder.encode(`{"crv":"${jwk.crv}","kty":"EC","x":"${jwk.x}","y":"${jwk.y}"}`);
...
export const jwkThumbprint = async (jwk: JsonWebKey, hash: (b: BufferSource) => Promise<ArrayBuffer>, decode: (s: ArrayBuffer) => string): Promise<string> => {
